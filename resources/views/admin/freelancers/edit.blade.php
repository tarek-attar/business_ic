...
@section('title')
    {{ __('site.Edit Freelancer') }} | {{ env('APP_NAME') }}
@endsection

@section('content')
    <!-- Page Heading -->
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 m-0 text-gray-800">{{ __('site.Edit Freelancer') }}</h1>
        <a href="{{ route('admin.freelancers.index') }}" class="btn btn-success px-5 ">{{ __('site.All Freelancers') }}</a>
    </div>
    @include('admin.errors')
    <form action="{{ route('admin.freelancers.update', $freelancer->id) }}" method="post" enctype="multipart/form-data">
        @csrf
        @method('put')
        <div class="mb-3">
