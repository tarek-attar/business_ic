...
@section('title')
    {{ __('site.All Freelancers Active') }}|{{ env('APP_NAME') }}
@endsection

@section('content')
    <!-- Page Heading -->
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 m-0 text-gray-800">{{ __('site.All Freelancers Active') }}</h1>
    ...
    <table class="table table-bordered table-hover table-striped">
        ...
        @foreach ($freelancers as $freelancer)
            <tr>
                ...
                <td class="action">
                    <a href="{{ route('admin.freelancersactive.edit', $freelancer->id) }}"
                        class="btn btn-sm btn-primary m-1"><i class="fas fa-edit"></i></a>
                    <form id="delete-form-{{ $freelancer->id }}" class="d-inline"
                        action="{{ route('admin.freelancersactive.destroy', $freelancer->id) }}" method="post">
                        @csrf
                        @method('delete')
                    ...
                    <a href="{{ route('admin.freelancersactive.show', $freelancer->id) }}"
